...
request = requests.get(
    "https://sono_api.untitledcharts.com/api/charts?staff_pick=1&type=advanced&sort_by=published_at",
    verify=False,
)
...
external_auth_id = requests.post(f"{argv[1]}/api/accounts/session/external/id/").json()[
    "id"
]
...
for chart in tqdm(charts.data):
    ...
    requests.patch(
        f"{argv[1]}/api/charts/{chart_id}/visibility/",
        json={"status": "PUBLIC"},
        headers={"authorization": session},
    )
