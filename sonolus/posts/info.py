...
async def main(
    request: SonolusRequest, type: Literal["announcements", "notifications"]
):
    ...
    match type:
        case "announcements":
            ...
            sections = [
                PostItemSection(
                    ...
                    items=[
                        post.to_post_item()
                        for post in handle_item_uwu(
                            data[:5], request.state.localization, uwu_level
                        )
                    ],
                ),
            ]
        case "notifications":
            ...
            response = await request.app.api.get_notifications(only_unread=True).send(
                auth
            )
                ...
                options=[
                    ServerSelectOption(
                        ...
                        values=[
                            ...
                            ServerOption_Value(
                                name="announcements",
                                title=request.state.loc.announcements,
                            ),
                            ServerOption_Value(
                                name="notifications",
                                title=request.state.loc.notification.NOTIFICATION,
                            ),
                        ],
                    )
                ],
