...
async def submit(request: SonolusRequest, item_name: str, data: GenericActionRequest):
    ...
    delete_response = await request.app.api.delete_leaderboard_record(
        chart_name, record_id
    ).send(auth)
    ...
    return ServerSubmitItemActionResponse(key="", hashes=[], shouldRemoveItem=True)
