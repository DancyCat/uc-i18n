...
async def staff_pick(
    auth: str,
    request: SonolusRequest,
    item_name: str,
    type: Literal["staff_pick_add", "staff_pick_delete"],
    locale: Loc,
) -> ServerSubmitItemActionResponse:
    response = await request.app.api.staff_pick_chart(
        item_name, True if type == "staff_pick_add" else False
    ).send(auth)
