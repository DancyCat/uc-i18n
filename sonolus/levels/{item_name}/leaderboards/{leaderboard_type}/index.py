...
async def info(
    item_name: str, leaderboard_type: leaderboard_type, request: SonolusRequest
):
    response = await request.app.api.get_leaderboard_info(
        item_name, leaderboard_type
    ).send(request.headers.get("Sonolus-Session"))
    ...
    return ServerItemLeaderboardDetails(
        topRecords=handle_item_uwu(
            await request.app.run_blocking(
                response.data.to_record_list, leaderboard_type
            ),
...
async def list(
    ...
    response = await request.app.api.get_leaderboards(
        item_name, leaderboard_type, page
    ).send(request.headers.get("Sonolus-Session"))
    ...
    return ServerItemLeaderboardRecordList(
        ...
        records=handle_item_uwu(
            await request.app.run_blocking(
                response.data.to_record_list, leaderboard_type, page=page
            ),
            request.state.localization,
            request.state.uwu,
        ),
...
async def leaderboard_record_info(item_name: str, name: str, request: SonolusRequest):
    ...
    replay_response = await request.app.api.get_leaderboard_record(
        item_name, int(name.rsplit("-", maxsplit=1)[-1])
    ).send(auth)
    ...
    replay_item = await request.app.run_blocking(
        replay_response.data.to_replay_item, request
    )

    return ServerItemLeaderboardRecordDetails(
        replays=handle_item_uwu(
